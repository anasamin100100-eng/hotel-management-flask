...
app = Flask(__name__)
app.secret_key = os.environ.get("HOTEL_SECRET", "dev_secret_key")
...
def calculate_dynamic_price(base_price):
    """
    Lightweight price function used for search & display only
    (no tax, no breakdown)
    """
...
def admin_dashboard():
    ...
    if request.method == "POST":
        ...
        r = Room(room_type=room_type, base_price=base_price, description=desc, is_available=True)
        db.session.add(r)
        db.session.commit()
    ...
    return render_template("admin_dashboard.html", rooms=rooms, reservations=reservations, user=user)
...
def edit_room(room_id):
    ...
    if request.method == "POST":
        ...
        # Optionally allow updating availability too
...
def search():
    ...
    if request.method == "POST":
        ...
        for r in rooms:
            ...
            r.price_today = price_today  # attach for template
...
# Book (GET shows booking form; POST creates reservation)
...
def invoice(reservation_id):
    ...
    return render_template("invoice.html", reservation=res, room=room, payment=pay, user=user)
...
def pay_page(reservation_id):
    ...
    if request.method == "POST":
        ...
        p = Payment(reservation_id=resv.id, amount=amount, method=method, status="paid")
        resv.paid = True
        db.session.add(p)
        db.session.commit()
...
# API-like endpoints (for programmatic use)
